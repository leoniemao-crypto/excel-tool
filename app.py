...
with tab1:
    ...
    with c1:
        ...
        st_tax_id = st.text_input("商店統編", value="24941093", key=f"st_tax_id_{cid}")
    with c2:
        st_address = st.text_input("商店地址", value="台北市南港區", key=f"st_address_{cid}")
        st_phone1 = st.text_input("商店電話 1", value="0912345678", key=f"st_phone1_{cid}")
    with c3:
        st_phone2 = st.text_input("商店電話 2", value="02-6631-5166", key=f"st_phone2_{cid}")
        ...
        with st.expander(f"🔹 點我填寫第 {i} 項商品內容", expanded=(i == 1)):
            ...
            with p_c2:
                ...
                if validity_type == "常態商品 (系統自訂50天)":
                    ...
                    with vd1: start_date_val = st.date_input(f"上架販售初日 ({i})", value=t_now, key=f"s_date_{i}_{cid}")
                    with vd2: end_date_val = st.date_input(f"兌換最後截止日 ({i})", value=t_now + timedelta(days=45), key=f"e_date_{i}_{cid}")
                    start_d = start_date_val.strftime("%Y/%m/%d")
                    end_d = end_date_val.strftime("%Y/%m/%d")
                    days_diff = (end_date_val - start_date_val).days
                    if days_diff > 50: st.warning(f"⚠️ 提示：目前相差 {days_diff} 天已超過 50 天（系統仍可正常導出）。")
                    else: st.success(f"✅ 符合官方常規（目前相差 {days_diff} 天）")
                
                format_type = st.selectbox(f"選擇要帶入的官方兌換時間文案格式 ({i})", ["格式一：• 本券兌換期間為（YYYY/MM/DD 至 YYYY/MM/DD）", "格式二：• 本券可兌換（商品名稱/規格），兌換期間為購買當日起至50日止。", "格式三：• 本券可兌換（商品名稱/規格），兌換期間為（YYYY/MM/DD 至 YYYY/MM/DD）"], key=f"format_type_{i}_{cid}")
                display_name = p_name if p_name else f"（第{i}項商品名稱/規格）"
                if "格式一" in format_type: final_validity_text = f"• 本券兌換期間為（{start_d} 至 {end_d}）"
                elif "格式二" in format_type: final_validity_text = f"• 本券可兌換（{display_name}），兌換期間為購買當日起至（兌換天數50日）日止。"
                else: final_validity_text = f"• 本券可兌換（{display_name}），兌換期間為（{start_d} 至 {end_d}）"
                st.code(final_validity_text, language="text")

                st.write("**🛒 商品販售時間**")
                sell_type = st.radio(f"販售屬性 ({i})", ["常態商品 (填無)", "季節性商品 (填日期區間)"], key=f"sell_type_{i}_{cid}")
                final_sell_time = "無" if sell_type == "常態商品 (填無)" else f"{st.date_input(f'販售開始日 ({i})', value=t_now, key=f's_s_{i}_{cid}').strftime('%Y/%m/%d')} 至 {st.date_input(f'販售結束日 ({i})', value=t_now+timedelta(days=14), key=f's_e_{i}_{cid}').strftime('%Y/%m/%d')}"

                st.write("**🏷️ 商品折扣時間**")
                discount_type = st.radio(f"折扣屬性 ({i})", ["原價販售 (填無)", "限定折扣 (填日期區間)"], key=f"discount_type_{i}_{cid}")
                final_discount_time = "無" if discount_type == "原價販售 (填無)" else f"{st.date_input(f'折扣開始日 ({i})', value=t_now, key=f'd_s_{i}_{cid}').strftime('%Y/%m/%d')} 至 {st.date_input(f'折扣結束日 ({i})', value=t_now+timedelta(days=7), key=f'd_e_{i}_{cid}').strftime('%Y/%m/%d')}"
