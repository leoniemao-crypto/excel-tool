        ...
        with st.expander(f"🔹 點我填寫第 {i} 項商品內容", expanded=(i == 1)):
            ...
            with p_c2:
                ...
                if validity_type == "常態商品 (自訂天數，最大150天)":
                    ...
                    # 💡 優化 3：修正格式二文案，將 xx日 改為自動計算出來的最後日期日期（end_d）
                ...
                # 💡 優化 1：商品販售時間日期「全自動預設對齊」兌換起始日與截止日，且保留點擊日曆微調空間
                st.write("**🛒 商品販售時間設定**")
                s1, s2 = st.columns(2)
                with s1:
                ...
                # 💡 優化 2：限定折扣時間日期「全自動預設對齊」兌換起始日與截止日，且保留點擊日曆微調空間
                st.write("**🏷️ 商品折扣時間設定**")
                d1, d2 = st.columns(2)
                with d1:
